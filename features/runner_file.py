...
def exec_file(filename, globals={}, locals=None):
    ...
    if sys.version_info[0] == 3:
        ...
        exec_file((filename, globals, locals))
...
class Runner(object):
    ...
    def setup_paths(self):
        ...
        while True:
            if self.config.verbose:
                print
                'Trying base directory:', new_base_dir
        ...
        if new_base_dir == root_dir:
            if self.config.verbose:
                if not self.config.paths:
                    print
                    'ERROR: Could not find "steps" directory. Please ' \
                    'specify where to find your features.'
                else:
                    print
                    'ERROR: Could not find "steps" directory in your ' \
